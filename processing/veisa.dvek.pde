...
class CLine implements Agent {
	...
	public void render(){
		...
		for (int i = 1; i < points.length-1; i++) {
			...
			float nz = noise(noiz);
			float px = random(points[i].x-3,points[i].x+3);
			float py = random(points[i].y-3,points[i].y+3);
			float tx = random(points[i].tx-3,points[i].tx+3);
			float ty = random(points[i].ty-3,points[i].ty+3);

			pushMatrix();

			translate(points[i].x, points[i].y);
			rotate( radians( rot ) );
			line(0, 0, tx-px, ty-py);
